...
front$CellRange <-as.factor(paste0("Perceptual range: ",front$CellRange))
front$Preference <-as.factor(paste0("Preference: ", front$Rate))
front$Emigration<- revalue(front$Scen, c("Pers + public" = "Informed",
                                     "Non-informed" = "Non-informed"))


tiff("Figures/Figure 4 - Front extent.tif",res=600,compression="lzw", width=5000,height=3000)
frontgg<-ggplot(front,aes(x=gen,y=Frontm,group=interaction(Emigration,ProspPatch),fill=ProspPatch))+
